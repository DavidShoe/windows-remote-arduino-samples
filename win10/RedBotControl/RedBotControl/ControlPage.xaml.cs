...
namespace RedBotControl
{
    ...
    public sealed partial class ControlPage : Page
    {
        ...
        private const byte HEARTBEAT_LED_PIN = 13;
        ...
        {
            arduino.digitalWrite(HEARTBEAT_LED_PIN, PinState.HIGH);
            App.firmata.beginSysex(0x65);  // Redbot
            App.firmata.appendSysex(0x01); // motor
            App.firmata.appendSysex(255);  // speed
            App.firmata.endSysex();

            LeftMotorSpeed = 0;
            RightMotorSpeed = 0;

            _LeftMotorSlider.Value = 255;
            _RightMotorSlider.Value = 255;
        ...
        {
            arduino.digitalWrite(HEARTBEAT_LED_PIN, PinState.LOW);
            App.firmata.beginSysex(0x65);  // Redbot
            App.firmata.appendSysex(0x01); // motor
            App.firmata.appendSysex(0);  // speed
            App.firmata.endSysex();

            LeftMotorSpeed = 0;
            RightMotorSpeed = 0;

            _LeftMotorSlider.Value = 0;
            _RightMotorSlider.Value = 0;
        ...
        private void OnLeftScrollChanged(object sender, Windows.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs e)
        {
            int value = (int)e.NewValue;
            byte speed = (byte)Math.Abs(value);

            if (Math.Abs(LeftMotorSpeed - value) > 5)
            {
                App.firmata.beginSysex(0x65);  // Redbot
                if (value >= 0)
                {
                    App.firmata.appendSysex(0x02); // left motor
                }
                else
                {
                    App.firmata.appendSysex(0x04); // left motor reverse
                }
                App.firmata.appendSysex(speed);  // speed
                App.firmata.endSysex();
        ...
        private void OnRightScrollChanged(object sender, Windows.UI.Xaml.Controls.Primitives.RangeBaseValueChangedEventArgs e)
        {
            int value = (int)e.NewValue;
            byte speed = (byte) Math.Abs(value);

            if (Math.Abs(RightMotorSpeed - value) > 5)
            {
                App.firmata.beginSysex(0x65);  // Redbot
                if (value >= 0)
                {
                    App.firmata.appendSysex(0x03); // right motor
                }
                else
                {
                    App.firmata.appendSysex(0x05); // right motor reverse
                }
                App.firmata.appendSysex(speed);  // speed
                App.firmata.endSysex();
